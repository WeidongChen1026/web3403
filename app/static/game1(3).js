      ...
      function val_check(){
        ...
        }else if(parseInt(ee)<1 || parseInt(ee)>99){
          $("#tips").text("Please enter a number between 1 and 99!");
        }else{
      ...
      function showRank(){
        $.ajax({
          type: 'GET',
          url: "http://127.0.0.1:5000/rank_list",
          data: '',
          dataType: 'json',
          success: function(data) {
            ...
            un_list = data.un_list
            rate_list = data.rate_list

            for(var i =0;i<data.un_list.length;i++){
              var tr = document.createElement("tr");
              var td_na = document.createElement("td")
              td_na.innerHTML= un_list[i];
              var td_nu = document.createElement("td")
              td_nu.innerHTML= rate_list[i];
